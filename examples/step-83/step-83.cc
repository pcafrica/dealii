...
namespace Step83
{
  ...
  public:
    ...
    void run(unsigned int timestep_number);
  ...
  void CathodeRaySimulator<dim>::create_particles()
  {
    ...
    std::cout << particle_handler.n_global_particles() << std::endl;
  ...
  template <int dim>
  void
  CathodeRaySimulator<dim>::update_timestep_size(unsigned int timestep_number)
  {
  ...
  template <int dim>
  void
  CathodeRaySimulator<dim>::output_results(const std::string &basename,
                                           unsigned int timestep_number) const
  {
    {
      ...
      data_out.set_flags(
        DataOutBase::VtkFlags(time.get_current_time(), timestep_number));
  ...
  template <int dim>
  void CathodeRaySimulator<dim>::run(unsigned int timestep_number)
  {
    if (timestep_number == 0)
      {
        make_grid();
    ...
    do
      {
        std::cout << "Timestep " << timestep_number + 1 << std::endl;
        ...
        n_recently_lost_particles = 0;
        update_timestep_size(timestep_number);
        move_particles();

        time.advance_time();
        ++timestep_number;

        output_results((timestep_number == 0) ? "output" : "restart",
                       timestep_number);
    ...
    {
      {
        ...
        cathode_ray_simulator.run(0);
      }

      {
        Step83::CathodeRaySimulator<2> cathode_ray_simulator;
        cathode_ray_simulator.run(1);
      }
    }
