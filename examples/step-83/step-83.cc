 ...
 * Based on step-19 by Wolfgang Bangerth, Colorado State University, 2021.
 */
...
namespace Step83
{
  ...
  // the checkpoint and restart functions. The run function
  // has also been modified to enable simulation restart.
  template <int dim>
  ...
  public:
    ...
    void run(const bool simulation_restart);
  ...
  private:
    ...
    void output_results(const std::string &basename) const;
  ...
  // @sect4{CathodeRaySimulator::constructor}
  
  // The constructor of the CathodeRaySimulator is identical
  // to step-19
  ...
  // since the electric field is solved for at the beggining of each timestep.
  // However, for pedagological purposes, we save the solution vector.
  template <int dim>
  ...
  // The main functions of the CathodeRaySimulator are identical
  // to step-19.
  ...
  void CathodeRaySimulator<dim>::make_grid()
  {
    ...
    triangulation.refine_global(1); // TODO
                 ...
                 fe_face_values.quadrature_point_indices())
              {
                ...
                if ((E * fe_face_values.normal_vector(q_point) < 0) &&
                    (E.norm() > Constants::E_threshold))
                  {
                    ...
                    std::cout << next_unused_particle_id << ", " << location
                              << ", " << particle_handler.n_global_particles()
                              << std::endl;
                  }
              }
    ...
    if (time.get_step_number() > 0)
      {
        ...
        std::cout << min_cell_size_over_velocity << std::endl;
  ...
  template <int dim>
  void
  CathodeRaySimulator<dim>::output_results(const std::string &basename) const
  {
    {
      ...
      std::ofstream output(basename + "-solution-" +
...

    {
      ...
      std::ofstream output(basename + "-particles-" +
                           Utilities::int_to_string(time.get_step_number(), 4) +
                           ".vtu");
  ...
  // driver. The driver takes a single function to indicate if the simulation
  // is a restart. If it is not a restart, the mesh is set-up and the problem is solved 
  // like in step-19. If it is a restart, then the mesh, the solution vector, the time object
  // and the particles.
  template <int dim>
  void CathodeRaySimulator<dim>::run(const bool simulation_restart)
  {
    if (!simulation_restart)
      {
        make_grid();

        const unsigned int n_pre_refinement_cycles = 0; // TODO
    ...
    do
      {
        ...
        std::cout << "___" << particle_handler.n_global_particles()
                  << std::endl;
        ...
        output_results((simulation_restart) ? "output" : "restart");
        ...
        // We checkpoint the simulation after 2e-5 secondes have elapsed.
        // and stop the simulation.
        if (!simulation_restart && time.get_current_time()>2.e-5)
        {
          checkpoint();
          break;
        }
    ...
    {
      {
        ...
        cathode_ray_simulator.run(false);
      }

      {
        //Step83::CathodeRaySimulator<2> cathode_ray_simulator;
        //cathode_ray_simulator.run(true);
      }
    }
